...
(function(exports) {
    ...
    function countWords(url) {
        return fetch(url).then(r=>r.json()).then(r=>{
            var newarr = {}  
            r['articles'].forEach(function(element){
                newarr[element['_id']] = element['text'].split(" ").length;
            })
            return newarr
        })
    }
    ...
    function getLargest(url) {
        return fetch(url).then(r=>r.json()).then(r=>{
            var newarr = []  
            r['articles'].forEach(function(element){
                newarr.push ( [element['_id'],element['text'].split(" ").length])
            })

            newarr.sort((a,b)=>(b[1]-a[1]));
            return newarr[0][0].toString();

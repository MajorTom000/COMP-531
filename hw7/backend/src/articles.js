...
const addArticle = (req, res) =>{
    ...
    //     const img = fields.image!='' ? fields.image : undefined
    const id = (new ObjectId()).toHexString()
    const text = req.body.text
    const img = req.body.image || req.body.img

    var newArticle = new Article({_id: id ,author:req.user, date: (new Date()).toUTCString(), text, img, comments:[]})
    ...
    Article.find({_id: req.params.id}).exec(function(err, articles){
        ...
        if (req.body.commentId){
            if (req.body.commentId == -1){
                ...
                const newcomment = new Comments({commentId: newcommentid, author:req.user, date: (new Date()).toUTCString(), text: req.body.text})
...
const returnUserFeed = (req, res, username) => {
    Profile.find({username}).exec(function(err,objects){
        ...
        Article.find({author: {$in : users}}).exec(function(e, docs){
            if (e){
                console.log(err)
                res.status(500).send('Internal Server Error')
                return
